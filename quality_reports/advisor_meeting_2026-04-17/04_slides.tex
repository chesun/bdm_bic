...
\begin{frame}{Two ways to run the test}
\small
\begin{tabular}{@{}p{0.18\linewidth} p{0.36\linewidth} p{0.36\linewidth}@{}}
\toprule
 & A & B \\
\midrule
Menu & 11 options & 11 options \\[0.3em]
Display & event-contingent pairs; mechanism hidden & native p-BDM framing \\[0.3em]
Arm & separate (between-subject) & separate (between-subject) \\[0.3em]
$\theta$ variation & within-subject; values TBD & same \\[0.3em]
$N$ & 300 & 300 \\
\bottomrule
\end{tabular}
...
\vspace{0.8em}
\footnotesize
Scope: BIC test for p-BDM only. A parallel pure-incentives test for MPL would require its own info/no-info and pure-incentives arms --- a separate study, not this one.
\end{frame}
...
\item UJS (CR over justifiable actions).
\end{enumerate}

\vspace{0.7em}
A strips the mechanism: pass A rules out (1). B preserves the p-BDM mechanism: fail B after pass A points to (3) or (2). Running both within-subject gives the sharpest A-vs-B contrast.

\vspace{0.8em}
\begin{block}{}
A, B, or both (within-subject)? Is there a fourth design we should consider?
\end{block}
\end{frame}
