...
Brown and Healy (2018): MPL presentation format determines whether IC survives subjects' behavior.
...
\vspace{0.6em}
Separated format (one row per screen, random order) breaks step 1: compound structure never salient, so ROCL is not triggered and non-EU preferences have no mechanism-level consequence.

\vspace{0.5em}
\footnotesize
Empirical test (B\&H 2018, risk elicitation): list $p = 0.041$ (IC rejected); separated $p = 0.697$ (IC preserved).
\end{frame}
...
\vspace{0.7em}
\footnotesize
List formats inherit the B\&H ROCL concern (previous slide). Two-stage formats (list or separated) hit a structural dilemma --- next slide.
\end{frame}

\begin{frame}{Two-stage formats: the MS/Stage-2 dilemma}
Any two-stage design narrows Stage 2's probability range based on Stage 1's crossing. If Stage 1 multi-switches (MS), there is no clean resolution:

\vspace{0.5em}
\begin{itemize}
\item \textbf{Force single-switching in Stage 1.} Stage 2 implementable, but censors MS behavior. C\&K 2025 benchmark: 29.7--43.7\% MS rates; forcing SS throws that data away and silently pushes subjects into patterns they would not have chosen.
\item \textbf{Allow MS, pick a crossing rule} (first, median, etc.). Arbitrary and breaks IC --- subject does not know ex ante which row determines payment, so the random-incentive guarantee fails.
\item \textbf{Skip Stage 2 for MS subjects.} Asymmetric rounds across subjects; precision gain lost precisely for the subjects whose data is most informative about inconsistency.
\end{itemize}

\vspace{0.4em}
Stage 2 MS within $\sim$1pp spacing is harder still: likely indifference or noise, but operationally still no crossing.

\vspace{0.4em}
\footnotesize
Applies equally to two-stage list (Holt \& Smith) and two-stage separated --- structural property of any design that narrows based on Stage-1 behavior, not list-specific.
\end{frame}

\begin{frame}{Question 3: MPL format}
Working lean: coarse separated (15--20 rows, one per screen, random order) --- the belief analog of C\&K's UJS-E mechanism. Works for our BIC test at 5pp precision, but does not generalize to precision-sensitive belief elicitation.

\vspace{0.5em}
\begin{block}{}
What is your view on MPL format for belief elicitation, given the precision-vs-interpretability tradeoff?
\end{block}

\begin{block}{}
B\&H's IC-preservation result used risk preferences with known lotteries. Our event bet is subjective. Does the cross-row transfer hold, or do we need an auxiliary B\&H arm?
\end{block}
\end{frame}
