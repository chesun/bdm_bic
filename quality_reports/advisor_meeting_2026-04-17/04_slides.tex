...
\vspace{0.8em}
Contribution: identify the mechanism behind BDM's failure, test UJS's design implication empirically in belief elicitation, and measure the complexity-by-mechanism interaction. The replication also extends DVW's p-BDM finding to four values of $\theta$.
\end{frame}

\begin{frame}{Hypotheses}
\begin{description}
\item[H1.] BDM fails behavioral IC, via two weak conditions:
\begin{itemize}
...
\item Is UJS the right framing for H2, or should the mechanism claim be contingent reasoning? UJS is cleanly identified by the design; CR would need a direct manipulation.
...
\begin{frame}{Proposal C: Example at $\theta = 0.2$ (MPL side)}
p-BDM side: same as Proposal A.

\vspace{0.6em}
MPL side: 10 binary choices, one per screen, random order. One row:

\vspace{0.3em}
\begin{quote}
Urn: 20 red, 80 blue. $P(\text{red}) = 0.2$. Pick one:
\begin{itemize}
\item[(A)] Event bet: win \$$H$ if red.
\item[(B)] 40\% lottery: win \$$H$ with probability $0.4$.
\end{itemize}
\end{quote}

\vspace{0.3em}
\footnotesize
Rows span $r \in \{0.05, 0.1, 0.15, 0.2, 0.25, 0.3, 0.4, 0.5, 0.7, 1.0\}$. Correct: event bet if $r < \theta$, $r$-lottery if $r > \theta$. Single crossing at $\theta$.

\vspace{0.3em}
If MPL pure-incentives pass rate $\gg$ p-BDM pure-incentives pass rate, the format-level UJS effect is isolated from belief formation.
\end{frame}

\begin{frame}{Three ways to run the test}
\small
\begin{tabular}{@{}p{0.12\linewidth} p{0.25\linewidth} p{0.25\linewidth} p{0.28\linewidth}@{}}
\toprule
 & A & B & C \\
\midrule
Menu & 11 options & 11 options & 11 opts + 10 rows \\[0.3em]
Display & event-contingent pairs; mechanism hidden & native p-BDM framing & A-style + MPL rows \\[0.3em]
$\theta$ & $\{0.2, 0.4, 0.6, 0.8\}$ & same & same \\[0.3em]
Integration & within-subject & within-subject & between-subject \\[0.3em]
MPL side & no & no & yes \\[0.3em]
$N$ & 300 & 300 & 600 \\
\bottomrule
\end{tabular}
\end{frame}

\begin{frame}{Question 2: which proposal?}
Three competing accounts of the 69\% result:
\begin{enumerate}
\item EV-calculation failure.
\item Ambiguity aversion or non-EU.
\item UJS.
\end{enumerate}
...
\vspace{0.8em}
\begin{block}{}
Which of A, B, or C identifies UJS most sharply against the alternatives? Is there a fourth design we should consider?
\end{block}
\end{frame}
