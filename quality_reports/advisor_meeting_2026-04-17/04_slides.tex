...
\vspace{0.7em}
\footnotesize
List formats give precision at low burden but inherit the ROCL concern. Two-stage variants (list or separated) hit an MS/Stage-2 dilemma: enforcing single-switching in Stage 1 censors MS, while allowing MS breaks Stage-2 implementability. Coarse separated sidesteps both: MS is observable and reported descriptively per C\&K 2025; format preserves IC per B\&H 2018.
\end{frame}

\begin{frame}{Question 3: MPL format}
Coarse separated: 15 to 20 rows, one per screen, random order. This is the belief analog of C\&K's UJS-E mechanism (they run it as a descending-price clock; same game structure per their characterization).

\vspace{0.5em}
\begin{block}{}
B\&H tested risk preferences with known lotteries in each row. Our event bet is ambiguous (subjective $\pi$). ROCL operates across rows; ambiguity operates within a row, and format does not close it. Does the cross-row transfer hold, or do we need an auxiliary B\&H transfer arm (100--150 subjects)?
\end{block}

\begin{block}{}
Two-stage formats (list or separated) are ruled out: MS in Stage 1 has no implementable resolution (force SS censors; crossing rules break IC; skip-Stage-2 is asymmetric). Does this reasoning hold?
\end{block}
\end{frame}
